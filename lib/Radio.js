...
module.exports = React.createClass({
  ...
  render: function () {
    ...
    var options = (props.options.items||[]).map(function (option, index) {
      ...
      return (
        <div value={option.value} className={className.trim()} tabIndex="0" key = {index} ref = {index} onClick={option.disabled?null:self.onClick.bind(self, index)} onKeyDown = {self.onKeyDown.bind(self, index)} onFocus = {self.onFocus.bind(self, index)} onBlur = {self.onBlur.bind(self, index)} >
          <label>
            <em></em>
